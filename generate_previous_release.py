...
def write_previous_release(previous_app_filter_file, custom_tag=""):
    ...
    latest_tag = custom_tag or next(
        (tag for tag in tags_output if tag != "nightly"), None
    )
    ...
    if lines:
        try:
            with open(previous_app_filter_file, "w") as f:
                f.write("\n".join(lines))
            print(
                f"Successfully wrote previous release appfilter to {previous_app_filter_file}"
            )
        except IOError as e:
